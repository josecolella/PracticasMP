...
TipoBase Lista :: operator[] (const int index)
{
	if(index+1 <= 0 || index+1>= this->size){
		...
		for(ptrToList = listHead,i=1;ptrToList!= NULL;i++,ptrToList= ptrToList->getNext())
			if(i == index)
				return ptrToList->getData();

		}
	}


	ostream & operator << (ostream &out, const Lista &otro)
	{
		Node *tmp;
		int i;

		for(i=0,tmp = otro.getListHead(); tmp != NULL; tmp = tmp -> getNext(),i++)
			out << "Valor en " << "[" << i+1 << "]: " << tmp->getData() << endl;
	}


	istream & operator >> (istream &in, Lista &otro)
	{
		char str[25 + 1];
		TipoBase newData;

		cout << "Insertar un valor  (* para terminar): ";
		cin.getline(str,25+1);
		while(strcmp(str,"*") != 0)
		{
			newData = atoi(str);
			otro.size++;
			otro.Insertar(newData,otro.size);
			cout << "Insertar un valor (* para terminar): ";
			cin.getline(str,25+1);
		}
