...
* para el objeto Matriz2D_2. Recordamos que este objeto es diferente que Matriz2D_2, y
...
Matriz2D_2& Matriz2D_2 :: operator= (const Matriz2D_2 & otro)
{
    ...
    TipoBase& Matriz2D_2 :: operator() (const int nFils, const int nCols)
    {
    ...
    Matriz2D_2 Matriz2D_2 ::  operator+ (const Matriz2D_2 &otro)
    {
    	...
    	{
    		Matriz2D_2 tmp(this->rows);

    		for(int f=0;f<this->rows*this->columns;f++)
    			tmp.matrix[0][f] = this->matrix[0][f] + otro.matrix[0][f]; 
    ...
    bool Matriz2D_2 :: operator== (const Matriz2D_2 &otro)
    {
    	...
    	if(isSameSize)
    	{
    		for(int f=0;f<this->rows*this->columns && isSameContent;f++)
    			if(this->matrix[0][f] != otro.matrix[0][f])
    				isSameContent = false;
    	...
    	bool Matriz2D_2 :: operator!= (const Matriz2D_2 &otro)
    	{
    		...
    		if(!isSameSize)
    		{
    			for(int f=0;f<this->rows*this->columns && isNotSameContent;f++)
    				if(this->matrix[0][f] == otro.matrix[0][f])
    		...
    		ostream & operator << (ostream & out, const Matriz2D_2 & otro)
    		{
